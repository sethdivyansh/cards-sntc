  ...
  return (
      <div className="bg-white p-8 rounded-3xl shadow-lg text-center flex flex-col justify-center align-middle">
        ...
        <div className='flex justify-center align-middle'>
          <button className={`${colorVariantsWhatsapp[color]} rounded-3xl text-white flex justify-center align-middle p-2 my-3 w-1/2 font-semibold`} onClick={()=> window.open(whatsappLink, '_blank')}><FaWhatsapp size={24} className='mr-2'/>WhatsApp</button>
        </div>
        <div className={`flex justify-center space-x-4 mt-1 `}>
          <a href={instaLink} className={`${colorVariants[color]}`}><FaInstagram size={24} /></a>
          <a href={linkedinLink} className={`${colorVariants[color]}`}><FaLinkedin size={24} /></a>
          <a href="https://github.com" className={`${colorVariants[color]}`}><FaGithub size={24} /></a>          
          <a href="mailto:example@example.com" className={`${colorVariants[color]}`}><FaEnvelope size={24} /></a>
        </div>
      </div>
  );
};
